...
PanelWindow {
    ...
    function getIcon(win) {
        ...
        let name = "";
        try {
            name = win.initialClass || win.class_ || win.appId || "";
            if (name === "" && win.title) {
                name = win.title;
            }
        } catch (e) {
        ...
        if (cleanName.includes("thyripagan"))  cleanName = "telegram";
        else if (cleanName.includes("dolphin"))  cleanName = "dolphin";
        else if (cleanName.includes("alacritty")) cleanName = "alacritty";
        else if (cleanName.includes("librewolf")) cleanName = "librewolf";
        else if (cleanName.includes("code"))      cleanName = "vscode";
        else if (cleanName.includes("obs")) cleanName = "obs";
        else if (cleanName.includes("cava")) cleanName = "alacritty";
        else if (cleanName.includes("vlc")) cleanName = "vlc";
        else if (cleanName.includes("fl studio")) cleanName = "flstudio";
        else if (cleanName.includes(".")) {
            let parts = cleanName.split(".");
            cleanName = parts[parts.length - 1];
        }
        cleanName = cleanName.trim();
        return Qt.resolvedUrl("./icons/" + cleanName + ".svg");
    }
    ...
    Rectangle {
        ...
        Row {
            ...
            // Закріплені іконки:
            Repeater {
                model: ["alacritty", "dolphin", "librewolf", "steam", "telegram"]
